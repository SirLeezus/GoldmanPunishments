...
public class TempMuteCMD extends CustomCommand {
  private final Punishments punishments;

  public TempMuteCMD(Punishments punishments) {
  ...
  public void performSender(CommandSender sender, String[] args, Command command) {
    ...
    if (cachePlayers.isMuted(targetID)) {
      sender.sendMessage(Lang.PREFIX.getComponent(null).append(Lang.ERROR_ALREADY_MUTED.getComponent(new String[]{ColorAPI.getNameColor(targetID, targetString)})));
      return;
    }
    ...
    cachePlayers.tempMutePlayer(targetID, reason, time, muter);
    PlayerDataAPI.sendPlayerMessageIfOnline(targetID, Lang.PREFIX.getComponent(null).append(Lang.COMMAND_TEMP_MUTE_TARGET_MESSAGE.getComponent(new String[]{timeFormatted, reason})));
    Bukkit.getServer().sendMessage(Lang.PREFIX.getComponent(null).append(Lang.COMMAND_TEMP_MUTE_BROADCAST.getComponent(new String[]{ColorAPI.getNameColor(targetID, targetString), timeFormatted, reason})));
  }
